    ...
    def browse_library(self, category: Optional[str] = None, search: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if search:
                ...
                chatmodes = [cm for cm in chatmodes if 
                           search_lower in cm.get('name', '').lower() or 
                           search_lower in cm.get('description', '').lower() or
                           search_lower in ' '.join(cm.get('tags', [])).lower()]
            ...
            if search:
                search_lower = search.lower()
                instructions = [inst for inst in instructions if 
                              search_lower in inst.get('name', '').lower() or 
                              search_lower in inst.get('description', '').lower() or
                              search_lower in ' '.join(inst.get('tags', [])).lower()]
    ...
    def install_from_library(self, name: str, custom_filename: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if item_type == 'chatmode':
                ...
                with tempfile.NamedTemporaryFile(mode='w', suffix='.chatmode.md', delete=False, encoding='utf-8') as tmp:
                ...
                # Write to temp file to parse
                with tempfile.NamedTemporaryFile(mode='w', suffix='.instruction.md', delete=False, encoding='utf-8') as tmp:
                    tmp.write(file_content)
                    tmp_path = tmp.name
